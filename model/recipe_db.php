...
function get_recipes_by_allergy($category_id, $allergies) {
  ...
  $query = 'SELECT * FROM recipes
            WHERE recipes.categoryID = :category_id '.$string.'
            ORDER BY recipeID DESC';
  try {
  $statement = $db->prepare($query);
   $statement->bindValue(":category_id", $category_id);
...
function add_recipe($category_id, $recipeName, $glutenFree, $dairyFree, $peanutFree, 
    ...
    $statement->bindValue(':submittedBy', $submittedBy);
